...
import {
  ...
  Eye, EyeOff, Search, Key, ArrowRight, ExternalLink, Edit, CheckCircle2, Lock, X
} from 'lucide-react';
...
export const SuperAdminHospitalDashboard = () => {
  ...
  // Clicking a box filters staff directly on this page
  ...
  return (
    <SuperAdminHospitalContext hospitalId={hospitalId}>
      <div className="space-y-6 animate-fade-in pb-12">
        ...
        {/* Stat Cards Grid (Click to filter table directly on this page) */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
              <Users size={18} className="text-indigo-600" />
              Hospital Staff & Patient Counts (Click box to view details & credentials below)
            </h3>
          </div>
            ...
            {HOSPITAL_DASHBOARD_CARDS.map((card) => {
              ...
              return (
                <div
                  ...
                  title={`Click to filter and view ${title} credentials table below`}
                >
                  <StatCard
                    title={title}
                    value={format ? format(stats[key]) : (stats[key] ?? 0)}
                    subtitle="Click to view credentials & change password"
                    icon={icon}
                    color={color}
                  />
                </div>
