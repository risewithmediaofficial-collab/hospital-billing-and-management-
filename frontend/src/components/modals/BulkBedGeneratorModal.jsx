...
export const BulkBedGeneratorModal = ({ isOpen, onClose, blocks = [], floors = [], wards = [], onSuccess }) => {
  ...
  const [wardTariff, setWardTariff] = useState(null); // preview: inherited from ward
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setWardId(selId);
                      const found = wards.find((w) => String(w._id) === String(selId));
                      setWardTariff(found?.defaultDailyCharge ?? null);
                    }}
                    className="w-full glass-input rounded-xl px-3 py-2 text-xs font-semibold text-slate-900 border border-slate-200"
                  >
              ...
              {wardTariff !== null ? (
                <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 flex items-start gap-2.5">
                  ...
                  <div>
                    <p className="text-xs font-bold text-emerald-800">
                      All beds will be billed at <span className="font-extrabold text-emerald-700">₹{wardTariff}/day</span> (Ward tariff)
                    </p>
                    <p className="text-[11px] text-emerald-700 mt-0.5">
                      Tariff is automatically inherited from the selected ward. No separate room or bed charge needed.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                  <p className="text-[11px] text-slate-500 font-semibold">
                    ⭐ Select a Ward above to automatically apply its configured daily tariff to all generated beds.
                  </p>
                </div>
              )}
