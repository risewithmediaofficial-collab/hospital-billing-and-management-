...
export const WORKFLOW_ACTIONS = {
  ...
  [WORKFLOW_EVENTS.BILL_READY]: { sourceModule: 'billing', targetModule: 'billing', entityType: 'Invoice', idKey: 'invoiceId', actionType: 'VIEW_BILL_STATUS', route: (p) => withParams('/billing/dashboard', { invoiceId: p.invoiceId, patientId: p.patientId }) },
  [WORKFLOW_EVENTS.PAYMENT_COLLECTED]: { sourceModule: 'billing', targetModule: 'billing', entityType: 'Receipt', idKey: 'receiptId', actionType: 'VIEW_RECEIPT', route: (p) => withParams('/billing/dashboard?tab=RECEIPTS', { receiptId: p.receiptId, patientId: p.patientId }) },
...
const MESSAGE_TEMPLATES = {
  ...
  [WORKFLOW_EVENTS.STAFF_WENT_OFFLINE]:
    (p) => ({ title: 'Staff Offline', message: `${p.staffName || 'Staff Member'} (${p.role || 'Staff'}) has gone offline. Patients cannot be assigned to them until they go back online.`, type: 'SYSTEM_ALERT' }),
